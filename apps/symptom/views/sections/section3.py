...
from apps.symptom.models import Agency, Customer, HistoricalSection1, Eligibility, Therapist, PsychiatricEvaluation
...
def section_three_document_three_history(request,pk):
    customer=get_object_or_404(Customer, pk=pk)     
    context = {
        #'psichiatric_evaluations': PsychiatricEvaluation.objects.filter(encrypted_file__belongs_to=pk).order_by('-created_at'),
